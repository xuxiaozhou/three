...
class Comment extends Sprite {
  constructor(param) {
    ...
    this.image = param.image || false;
    this.lifeTime = param.lifeTime || parseInt(config.display.comment.lifeTime);
  ...
  async draw(canvasContext) {
    ...
    if (config.display.image && this.image) {
      // 图片弹幕
      let src = this.image;
      let imageObject = utils.imageAnalyzer.getImage(src);
      if (imageObject === null) {
        imageObject = await utils.imageAnalyzer.buildCache(src, 280);
      }
    ...
    if (this.cache === null) {
      ...
      utils.imageAnalyzer.regex = /\[IMG WIDTH=(.+?)\](.+?)\[\/IMG\]/ig;

      await utils.imageAnalyzer.test((ret, imageObject) => {
        ...
        this.cache.push({
          ...
          object: imageObject,
          position: nextPos,
          width: imageObject.width === 'HEIGHT' ? this.height : imageObject.width
        });
        nextPos += imageObject.width === 'HEIGHT' ? this.height : imageObject.width + spaceWidth;
