...
class DrawManager {
  ...
  public draw(ctx: CanvasRenderingContext2D, options: IDrawOptions) {
    ...
    if (this.spriteConfig.type === 'text') {
      ...
      const imageSpaceTop = (options.height - this.spriteConfig.imageSize) / 2;

      DrawManager.drawAvatar(
        ctx,
        {
          img: options.image,
          position: [this.spriteConfig.avatarSize + imageAndTextSpace + left, imageSpaceTop],
          imageHeight: this.spriteConfig.imageSize,
          imageWidth: this.spriteConfig.imageSize,
          radius: this.spriteConfig.radius,
        }
      );
    }
  ...
  private drawText(ctx: CanvasRenderingContext2D, {left = 0, top = 0}: { top: number, left: number }) {
    ctx.fillStyle = this.spriteConfig.fontColor || getRandomColor();
