...
class CommentFrame extends Frame {
  ...
  addSprite (info) {
    ...
    let font = info.font || `normal bold ${config.display.comment.fontSize}px PingFang-SC`
    let lifeTime = info.lifeTime || parseInt(config.display.comment.lifeTime)
    let height = info.height || parseInt(config.display.comment.fontSize) + 5
    let image = info.image || false
    let text = info.text
        ...
        for (let j = 0; j < this.layers[i].length; j++) {
          if (!this.layers[i][j].alive) {
            ...
            for (let l = j; l < this.layers[i].length; l++) {
              ...
              // console.log(this.idMap[this.layers[i][l].id].index)
            }
          }
        }
      }
